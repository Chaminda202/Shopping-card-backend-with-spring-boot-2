...
//	@CachePut(cacheNames = {"userAll", "userById"}, key = "#result.id")
	...
	@Override
	public UserResponse loginUser(String name, String password) throws ApplicationException {
		Optional<User> userOptional = this.userRepository.findByNameAndPassword(name, password);
		if (userOptional.isPresent()) {
			return buildResponse(userOptional.get());
		}
		throw new ApplicationException(this.appErrorConfig.getInvalidUsernamePassword());
	}
